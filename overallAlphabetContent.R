...
option_list = list(
  ...
  make_option(c("-w", "--plot_width"), type="numeric", default=12,
              help="plot width [default= %default]", metavar="numeric"),
...
read_fastas = function(fasta_path_vec) {
...
save_plot = function(p, out_path, width, height) {
  pdf(out_path, width)
  print(p)
  dev.off()
  return()
...
overall_frequency = function(fastas_vec,
  ...
  save_plot(p, out_path, plot_width, plot_height)
