...
option_list = list(
  ...
  make_option(c("-a", "--alphabet_vec"), type="character", default='A,C,G,T', 
              help="alphabet used (either DNA/RNA/protein or alphabet string separated by commas)", metavar="character"),
...
divide_bins = function(combined_fasta) {
  ...
  # combined_fasta = cbind(combined_fasta, t(middle_bins_mat))
...
create_alphabet_metagene = function(fasta_path_vec, match=NULL, alphabet_vec, sample_names_vec,
  ...
  if (is.null(sample_names_vec)) {
    sample_names_vec = as.character(seq(1,length(fastas),1))
  }
  else {
